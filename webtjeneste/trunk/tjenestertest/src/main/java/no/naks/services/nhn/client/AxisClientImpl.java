...
public class AxisClientImpl implements AxisClient {
	...
	public AxisClientImpl(String nHN_TEST) {
		super();
		if (nHN_TEST != null && !nHN_TEST.equals(""))
			NHN_TEST = nHN_TEST;
	}
	...
	public List connectToNHN(String url) {
		List<CommunicationParty> organisations = new ArrayList();
		...
		no.naks.services.nhn.client.adr.CommunicationPartyStub.Ping ping52 = new Ping();
		 ...
		 ArrayOfKeyValuePairOfstringstring searchArray = new ArrayOfKeyValuePairOfstringstring();
		   ...
		   no.naks.services.nhn.client.adr.CommunicationPartyStub.KeyValuePairOfstringstring valuePair14 = new KeyValuePairOfstringstring();
		      ...
		      valuePair3.setValue("101");			//Dette er koden for sykehus etc.
/*		 Disse andre kodene er ikke nødvendig:     
...
*/		      
		...
		search.setSearch(comSearch);
		 ...
		 try {
			 ...
			 CommunicationParty[] resultArray= null;
			try {
				results = adreg.search(search);
			} catch (RemoteException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			if (results != null)
				resultArray = results.getSearchResult().getResults().getCommunicationParty();
			int i = 0;
			...
			for(CommunicationParty p : resultArray){
				...
				if (type.toString().equals("Organization")||type.toString().equals("Department") ){
					...
					System.out.println(	p.getParentHerId()+ " "+p.getHerId()+" - "+p.getName()+ " "+type.toString()+ " "+i);
					organisations.add(p);
				}
