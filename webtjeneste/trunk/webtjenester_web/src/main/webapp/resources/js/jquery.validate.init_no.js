...
* Kommontar: Velger √• ha egen "doc.ready" for validation plugin, for det gj√∏r det enkelt √• bytte spr√•k, enten via javascrip, eller via XSLT p√• portlett-/framework-niv√•.
*            PS. Har ogs√• lagt "info-scriptene" i denne fila.
*
    ...
    if ($('#where-when-who-form').length !== 0) {
       ...
       $('#where-when-who-form').validate({
            ...
            showErrors: function(errorMap, errorList) {
             this.defaultShowErrors(); // Denne m√• v√¶re med, hvis ikke vil showErrors: fjerne de vanlige feilmeldingene.
             // datoHidden
       ...
       // Tvinger frem validering pÂ utvalgte felt nÂr side lastes.
       ...
       */
       ...
       // Tvinger frem validering pÂ "required" felt "spesialist". (Som ellers bare blir validert pÂ submit.)
       $('#where-when-who-form\\:spesialist').blur(function() {
            $('#where-when-who-form').validate().element('#where-when-who-form\\:spesialist');
       });
       // Tvinger frem validering pÂ "required" felt "eksisterende". (Som ellers bare blir validert pÂ submit.)
       $('#where-when-who-form\\:eksisterende').blur(function() {
            $('#where-when-who-form').validate().element('#where-when-who-form\\:eksisterende');
       });
       // Tvinger frem validering pÂ datoene i "datepicer". (Som ikke er en del av validatoren, og ikke har noen innebygd event knyttet til seg.)
       ...
       // GrÂe ut (og vica versa) sykehus, samt vise og skjule "Hvor skjedde hendelsen?":
       ...
       // GrÂe ut sykehus mm nÂr siden lastes hvis checked=true.
       ...
       // GrÂe ut, t¯mme og disable f¯dselsÂr:
...
if ($('#where-when-who-form\\:ingen').attr('checked')==='checked' || $('#where-when-who-form\\:flere').attr('checked')==='checked') {
    ...
    $('#where-when-who-form\\:year-of-birth').attr('disabled', 'disabled');  
       ...
       // Sjekke om validering skal trigges nÂr siden lastes.
    ...
    if ($('#incident-form').length !== 0) {
       ...
       // Tvinger frem validering p√• "required" felt. (Som ellers bare blir validert p√• submit.)
    ...
    if ($('#contact-form').length !== 0) {
       ...
       // Sjekke om validering skal trigges n√•r siden lastes.
       // P√• kontaktsiden finne btnKontroll ved f√∏rste treff, s√• sjekker p√• kontrollHidden istede.
       if ( $('#contact-form\\:kontrollHidden').attr('value') === 'true' || // NB! denne skal v√¶re 'false'
             $('#contact-form\\:phoneHidden').attr('value') === 'false' ||
             $('#contact-form\\:lederTelefon').attr('value') === 'false' ||
             $('#contact-form\\:emailHidden').attr('value') === 'false' ||
             $('#contact-form\\:gjentaHidden').attr('value') === 'false' ||
