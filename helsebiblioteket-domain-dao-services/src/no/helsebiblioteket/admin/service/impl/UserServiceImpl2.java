...
public class UserServiceImpl2 implements UserService {
	...
	private void saveUserRoleLineListForUser(User changedUser, User originalUser) {
		List<UserRoleLine> changedUserRoleLineList = null;
		List<UserRoleLine> originalUserRoleLineList = null;
		UserRoleLine userRoleLine = null;
		if (changedUser != null && changedUser.getRoleList() != null) {
			changedUserRoleLineList = new ArrayList<UserRoleLine>();
			userRoleLine = null;
			for (Role role : changedUser.getRoleList()) {
				userRoleLine = new UserRoleLine();
		...
		if (originalUser != null && originalUser.getRoleList() != null) {
			originalUserRoleLineList = new ArrayList<UserRoleLine>();
			userRoleLine = null;
			for (Role role : changedUser.getRoleList()) {
				userRoleLine = new UserRoleLine();
		...
		ModifiedListHelper<UserRoleLine> listHelper = new ModifiedListHelper<UserRoleLine>();
		List<UserRoleLine> deleteList = listHelper.getDeleteList(changedUserRoleLineList, originalUserRoleLineList);;
		List<UserRoleLine> insertAndUpdateList = listHelper.getInsertAndUpdateList(changedUserRoleLineList, originalUserRoleLineList);
		if (deleteList != null) {
			for (UserRoleLine line : deleteList) {
				roleDao.deleteUserRoleLine(line);
			}
		}
		if (insertAndUpdateList != null) {
			for (UserRoleLine line : insertAndUpdateList) {
