...
public class LinkCheckerPlugin extends TaskPlugin {
	...
	public void handleProperties(Properties taskProperties) {
		...
		if (taskProperties != null && !taskProperties.isEmpty()) {
			...
			try {
				saveFolder = Integer.parseInt(taskProperties.getProperty(TaskPropertyKeys.cmsSaveFolder.name()));
			} catch (NumberFormatException nfe) {
			...
			String cmsReceivers = taskProperties.getProperty(TaskPropertyKeys.cmsReceivers.name());
			StringTokenizer emailTokenizer = new StringTokenizer(cmsReceivers, ",");
